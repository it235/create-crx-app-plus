...
export async function createApp({
  ...
  const templateName = `${useFramework}-${useTypescript ? 'ts' : 'js'}-mv3`.toLowerCase()
  ...
  console.log()
  console.log(`Copying ${chalk.cyan(templateName)} template`)
  try {
    await downloadAndExtractExample(root, templateName)
  } catch (error) {
    console.log(error)
    console.log(`some wrong with copying ${chalk.cyan(templateName)} template`)
    process.exit(1)
  }
  console.log()
